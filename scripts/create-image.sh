...
DIR=chroot
PREINSTALL_PKGS=openssh-server,curl,cron,tar,gcc,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy,selinux-policy-default,firmware-atheros,debian-ports-archive-keyring,make
...
echo 'host0 /hostshare 9p trans=virtio,rw,_netdev 0 0' | sudo tee -a $DIR/etc/fstab
echo -en "127.0.0.1\tlocalhost\n" | sudo tee $DIR/etc/hosts
echo "nameserver 8.8.8.8" | sudo tee -a $DIR/etc/resolve.conf
echo "syzkaller" | sudo tee $DIR/etc/hostname
echo "@reboot root /hostshare/startup.sh >/dev/ttyS0 2>&1" | sudo tee $DIR/etc/cron.d/onreboot
ssh-keygen -f $RELEASE.id_rsa -t rsa -N ''
sudo mkdir -p $DIR/root/.ssh/
sudo mkdir -p $DIR/hostshare/
